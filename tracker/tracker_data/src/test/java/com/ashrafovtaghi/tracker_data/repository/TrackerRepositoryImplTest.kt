...
import com.ashrafovtaghi.tracker_data.remote.malformedFoodResponse
import com.ashrafovtaghi.tracker_data.remote.validFoodResponse
...
class TrackerRepositoryImplTest {
    ...
    @Test
    fun `Search food, valid response, return results`() = runTest {
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `Search food, invalid response, return results`() = runTest {
        mockWebServer.enqueue(
            MockResponse()
                .setResponseCode(403)
                .setBody(
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    @Test
    fun `Search food, malformed response, return results`() = runTest {
        mockWebServer.enqueue(
            MockResponse()
                .setBody(
                    malformedFoodResponse
                )
        )
